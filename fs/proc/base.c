...
static int proc_stuck_trace_show(struct seq_file *m, void *v)
{
	...
	printk(KERN_WARNING "fuyou,show...  ltt=%llu, mid=%llu, big=%llu\n",
		ltt_time, mid_time, big_time);
				...
				size_t count, loff_t *ppos)
{
	...
	if (count > sizeof(buffer) - 1)
		count = sizeof(buffer) - 1;
	if (copy_from_user(buffer, buf, count)) {
		trace_printk("uxc, proc_static_ux_write return -EFAULT\n");
		return -EFAULT;
	}
	err = kstrtoint(strstrip(buffer), 0, &static_ux);
	if (err) {
		trace_printk("uxc, proc_static_ux_write return err = %d\n", err);
		return err;
	}
	task = get_proc_task(file_inode(file));
	if (!task) {
		trace_printk("uxc, proc_static_ux_write return -ESRCH\n");
		return -ESRCH;
	}

	task->static_ux = static_ux != 0 ? 1 : 0;

	trace_printk("uxc, proc_static_ux_write taskpid = %d, static_ux = %d\n", task->pid, task->static_ux);
				...
				struct pid *pid, struct task_struct *task)
{
	...
	seq_printf(m, "window_index:%d,timestamp:%llu\n",
	...
	seq_printf(m, "overlod_flag:%016x\n", task->tli[0].tli_overload_flag);
...
static const struct pid_entry tgid_base_stuff[] = {
	...
	ONE("statm_as",   0444, proc_pid_statm_as),
#endif
